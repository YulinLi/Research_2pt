...
class SAC():
    ...
    def choose_action(self, s, eval=False):
        ...
        s_g_img4 = tool.get_gaussian(
            s_coord[0, 2].cpu().detach().numpy(), s_coord[0, 3].cpu().detach().numpy(), 4, 32, 128)
        """
        ...
        s_g_img4 = torch.FloatTensor(np.expand_dims(s_g_img4, 0)).cuda()
        s_g_img4 = s_g_img4.permute(0, 3, 1, 2)
        # concat full image and gaussian image
        s_h_img = torch.cat((s_img, s_g_img, s_g_img4), dim=1)
        # cropped image
        ...
        if B_s_coord > 1:
            for i in range(1, B_s_coord):
                grid_temp4 = tool.get_grid(
                    128, 128, s_coord[i, 2], s_coord[i, 3], 32, 32, 32, 32).cuda()
                grid4 = torch.cat((grid4, grid_temp4), dim=0)
        s_cropped4 = F.grid_sample(s_img, grid4)
        ...
        if eval == False:
            action, _, _ = self.actor.sample(s_h_img, s_cropped1, s_cropped4)
        else:
            _, _, action = self.actor.sample(s_h_img, s_cropped1, s_cropped4)
    ...
    def learn(self):
        ...
        s_g_img_batch4 = s_g_img4
        for i in range(1, B):
            s_g_img4 = tool.get_gaussian(s_coord[i, 2].cpu().detach(
            ).numpy(), s_coord[i, 3].cpu().detach().numpy(), 4, 32, 128)
            s_g_img4 = torch.FloatTensor(np.expand_dims(s_g_img4, 0)).cuda()
            s_g_img4 = s_g_img4.permute(0, 3, 1, 2)
            s_g_img_batch4 = torch.cat((s_g_img_batch4, s_g_img4), dim=0)
        # concat full image and gaussian image
        # TODO 1 is this right?
        s_h_img = torch.cat((s_img, s_g_img_batch, s_g_img_batch4), dim=1)
        ...
        sn_g_img4 = tool.get_gaussian(
            sn_coord[0, 2].cpu().detach().numpy(), sn_coord[0, 3].cpu().detach().numpy(), 4, 128, 128)
        sn_g_img4 = torch.FloatTensor(np.expand_dims(sn_g_img4, 0)).cuda()
        sn_g_img4 = sn_g_img4.permute(0, 3, 1, 2)
        ...
        sn_g_img_batch4 = sn_g_img4
        for i in range(1, B):
            sn_g_img4 = tool.get_gaussian(sn_coord[i, 2].cpu().detach(
            ).numpy(), sn_coord[i, 3].cpu().detach().numpy(), 4, 32, 128)

            sn_g_img4 = torch.FloatTensor(np.expand_dims(sn_g_img4, 0)).cuda()

            sn_g_img4 = sn_g_img4.permute(0, 3, 1, 2)
            sn_g_img_batch4 = torch.cat((sn_g_img_batch4, sn_g_img4), dim=0)
        # concat full image and gaussian image
        sn_h_img = torch.cat((sn_img, sn_g_img_batch, sn_g_img_batch4), dim=1)
        ...
        grid4 = tool.get_grid(
            128, 128, sn_coord[0, 2], sn_coord[0, 3], 32, 32, 32, 32).cuda()
        if B_sn_coord > 1:
            for i in range(1, B_sn_coord):
                grid_temp4 = tool.get_grid(
                    128, 128, sn_coord[i, 2], sn_coord[i, 3], 32, 32, 32, 32).cuda()
                grid4 = torch.cat((grid4, grid_temp4), dim=0)
        # 2.grid sample
        sn_cropped4 = F.grid_sample(sn_img, grid4)
        ...
        with torch.no_grad():
            a_next, logpi_next, _ = self.actor.sample(
                sn_h_img, sn_cropped1, sn_cropped4)
            q_next_target = self.critic_target(
                sn_h_img, sn_cropped1, sn_cropped4, a_next) - self.alpha * logpi_next
            q_target = r_ts + end_ts * self.gamma * q_next_target

        # Critic loss
        q_eval = self.critic(s_h_img, s_cropped1, s_cropped4, a_ts)
        ...
        a_curr, logpi_curr, _ = self.actor.sample(
            s_h_img, s_cropped1, s_cropped4)
        q_current = self.critic(s_h_img, s_cropped1, s_cropped4, a_curr)
